...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:pearl_milk_tea) { described_class.new('pearl milk tea', 16) }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(pearl_milk_tea.type).to eq('pearl milk tea')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(pearl_milk_tea.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:aloe_drink) { described_class.new('aloe', 24) }

      # remove the 'x' before running this test
      it 'is full' do
        expect(aloe_drink).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:vodka_shot) { described_class.new('vodka', 2) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(vodka_shot).not_to be_full
      end
    end
  end
end
