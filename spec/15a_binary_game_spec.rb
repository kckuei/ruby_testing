...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        invalid_input_1 = 'abcdefg'
        valid_input = '1'
        allow(game_input).to receive(:gets).and_return(invalid_input_1, valid_input)
      end
  ...
  describe '#display_binary_search' do
    ...
    context 'when game_over? is false five times' do
      ...
      it 'calls display_turn_order five times' do
        expect(game_display).to receive(:display_turn_order).with(search_display).at_most(5).times
        game_display.display_binary_search(search_display)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    subject(:new_game) { described_class.new(0, 100) }
