...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous value is 17' do
  ...
  describe '#play' do
    context 'when the random number is 7292037202829' do
      subject(:new_game) { described_class.new(7292037202829) }
    ...
    context 'when the random number is 777' do
      subject(:new_game) { described_class.new(777) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = new_game.play
        expect(result).to eq(7)
    ...
    context 'when the random number is 0' do
      subject(:new_game) { described_class.new(0) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = new_game.play
        expect(result).to eq(7)
